...
void bufs_single_output_action_group_apply(void){
	...
	CU_ASSERT(of1x_group_add(sw->pipeline.groups,OF1X_GROUP_TYPE_INDIRECT,grp_id,&buckets) == ROFL_OF1X_GM_SUCCESS);
...
void bufs_multiple_output_actions_group(void){
	...
	of1x_push_packet_action_to_group(ag, of1x_init_packet_action(OF1X_AT_OUTPUT,field,0x0));
	of1x_insert_bucket_in_list(buckets,of1x_init_bucket(0,1,0,ag));
	CU_ASSERT(of1x_group_add(sw->pipeline.groups,OF1X_GROUP_TYPE_ALL,grp_id,&buckets) == ROFL_OF1X_GM_SUCCESS);
